...
class KotlinDeprecatedMethodScannerTest {
    ...
    class ScanArrowCore {
        ...
        void findsDeprecatedMethods() throws IOException {
            ...
            assertThat(result.groupId()).isEqualTo("io.arrow-kt");
            assertThat(result.artifactId()).isEqualTo("arrow-core");
            assertThat(result.majorVersion()).isEqualTo("2");
            assertThat(result.deprecatedMethods()).isNotEmpty();
        }

        @Test
        void extractsMethodPatterns() throws IOException {
            ScanResult result = scanner.scan("arrow-core");

            assertThat(result).isNotNull();
            assertThat(result.deprecatedMethods())
                    .extracting(DeprecatedMethod::methodPattern)
                    .anyMatch(pattern -> pattern.contains("mapOrAccumulate"));
        }

        @Test
        void extractsReplacements() throws IOException {
            ScanResult result = scanner.scan("arrow-core");

            assertThat(result).isNotNull();
            assertThat(result.deprecatedMethods())
                    .extracting(DeprecatedMethod::replacement)
                    .anyMatch(replacement -> replacement.contains("mapValuesOrAccumulate"));
        }
    }
